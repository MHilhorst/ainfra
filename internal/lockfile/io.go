...
// WriteStable writes l, but when its content apart from GeneratedAt is
// identical to the lock already at path it keeps the existing GeneratedAt, so
// a re-resolve that changes nothing leaves the file byte-for-byte unchanged.
func WriteStable(path string, l *Lock) error {
	if existing, err := Read(path); err == nil {
		prior := existing.GeneratedAt
		existing.GeneratedAt = l.GeneratedAt
		a, errA := yaml.Marshal(existing)
		b, errB := yaml.Marshal(l)
