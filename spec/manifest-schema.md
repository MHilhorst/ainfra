...
version*, not the cache key itself. Claude Code resolves a plugin's version in
precedence order: the plugin's own `plugin.json`, then its marketplace entry,
then the git commit SHA, then `"unknown"` (see the plugins reference). It caches
the result at `~/.claude/plugins/cache/<marketplace>/<name>/<version>/` and
records it in `~/.claude/plugins/installed_plugins.json`, which `ainfra` reads
after install to warn when the resolved version diverges from the pin. The pin
itself is never passed to `claude plugin install`. To change the *actual* cache
key, bump the upstream `plugin.json`/marketplace entry.

Omitting `version:` selects the SHA-versioned flow: `ainfra` cannot know the
upstream's current SHA without a fetch, so an unpinned plugin always plans as an
update and lets `claude plugin update` decide whether anything is new. That is
the correct behavior for a plugin whose `plugin.json` declares no version — every
upstream commit is a new version, and pinning would defeat it.
