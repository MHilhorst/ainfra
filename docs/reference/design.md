...
| **Schema/docs drift** — a hand-maintained schema or doc slowly diverges from what the tool actually accepts | Common wherever reference docs are written by hand | The JSON Schema is *generated by reflection from the Go structs the loader uses* (`ainfra install --print-schema`). It cannot drift from the parser. |
...
- **Apply history** — every non-dry-run `ainfra install` appends one line per
  non-noop change to `.ainfra/history.jsonl`. Read the log directly (e.g. with
  `jq`); the standalone `history` verb is hidden through 0.x.
